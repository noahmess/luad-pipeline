...
| TIMER3.0 | Cui et al. (2025), web portal, purity-adjusted partial Spearman correlations (TCGA-LUAD, n = 516, purity adjustment on) | `R/07`, `R/16` (reference only) | **Source data for Figure 12**: `outputs/tables/TIMER3_NUP88_immune_correlations.csv` (NUP88) and `outputs/tables/TIMER3_LMNB2_immune_correlations.csv` (LMNB2). Each has columns `cancer, infiltrates, rho, p, adj.p`; 135 rows (1 NA), 11 deconvolution algorithms. Direct TIMER3.0 exports; supersede the old `S2 TABLE TIMER3 NUP88 LMBN2.xlsx` |
...
| **Figure 12 (NUP88/LMNB2 immune infiltration, TIMER3.0)** | no rendering script in the pipeline - the heatmap is drawn from the committed TIMER3.0 source data `outputs/tables/TIMER3_NUP88_immune_correlations.csv` and `outputs/tables/TIMER3_LMNB2_immune_correlations.csv` (purity-adjusted partial Spearman, 11 algorithms). The manuscript figure was produced externally and is kept as-is | Section 3.6 |
